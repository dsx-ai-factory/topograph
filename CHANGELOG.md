...
- Minerva provider (`minerva`) discovers switch-tree topology from the NVIDIA Minerva `POST /v1/export-topology` API. Like the NetQ provider, it reduces Clos fabrics into a canonical tree via `topology.Merger`; unlike NetQ, Minerva does not report tier numbers, so each device's tier is derived from its `role` — `server`, `leaf`, `spine`, and `super_spine` map to fixed tiers per Minerva's documented standard hierarchy, and devices with any other role (including deployment-specific "Custom" roles) take no part in the switch tree. Requested servers with no resolvable switch parent are grouped under a shared `no-topology` switch instead of being dropped. Authenticates via the `X-Api-Key` header using an `apiKey` credential. Does not populate `topology/block` — Minerva's export doesn't report NVLink domain membership, only per-server GPU attachments.
